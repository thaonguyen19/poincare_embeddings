...
def generate_pairs_closure(package_file, dataset, depth, sep='.'): 
    #package_file name has format "*_sorted"
    mapping = ddict(set) #map from higher order element to the all direct children in the hierarchy
    all_last_tokens = set()
    duplicate = set() #assume the immediate parent package names would be different
...
def process_duplicate_closure(duplicate_set, file_read, file_write, clique_type, depth, sep='.'):
    '''clique_type options: wo_clique, full_clique, basic_clique'''
    w = int(DEFAULT_WEIGHT/1)
    duplicate_dict = ddict(set)
    count_renamed_tokens = ddict(int)
...
if __name__ == '__main__':
    ...
    for package_file_sorted in [main_file]:#, debug_file]:
        ...
        duplicate_set, duplicate_file_name, tsv_package_file = generate_pairs_closure(package_file_sorted, 'train', depth=True)
        process_duplicate_closure(duplicate_set, package_file_sorted, tsv_package_file, clique_type=clique_type, depth=True)
